import React, { useState } from 'react';
import api from '../services/api'
import { useHistory } from 'react-router-dom'

const Form = (props: any) => {
    const history = useHistory()


    const [data, setData] = useState({
        name: '',
        description: '',
        photo: '',
        price: ''
    })

    const handleInputChange = (event: { target: { name: any; value: any; }; }) => {

        setData({
            ...data,
            [event.target.name]: event.target.value
        })
    }



    const sendData = async (event: any, data: any) => {

        event.preventDefault()
        let id: number = 0
        const dataStore = {
            "id": id++,
            "photo": data.photo.value,
            "name": data.name.value,
            "description": data.description.value,
            "price": data.price.value
        }
        localStorage.setItem('Product', JSON.stringify(dataStore))
        const response = await api.post('Product', dataStore);
        history.push('')

    }

    return (
        <>
            <div className="head">
                <img src="data:image/png;base64," alt="Logo Vtex" width='200px' height='auto' />
            </div>
            <form className="" onSubmit={() => sendData}>
                <h1>{props.title}</h1>
                <div>
                    <label htmlFor="name">Nome do Produto</label>
                    <input type="text" placeholder="Nome" className="" onChange={handleInputChange} name="name"></input>
                </div>
                <div>
                    <label htmlFor="description">Descrição do Produto</label>
                    <input type="text" placeholder="Descrição" className="" onChange={handleInputChange} name="description"></input>
                </div>
                <div>
                    <label htmlFor="photo">Link da imagem do Produto</label>
                    <input type="text" placeholder="Link da imagem" className="" onChange={handleInputChange} name="photo"></input>
                </div>
                <div>
                    <label htmlFor="price">Valor do Produto</label>
                    <input type="text" placeholder="Valor" className="" onChange={handleInputChange} name="price"></input>
                </div>
                <button type="submit" className="btn">Enviar</button>
            </form>

        </>
    );
}

export default Form;